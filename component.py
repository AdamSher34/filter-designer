...
class VALUE_SEQUENCE(Enum):
    EXACT = None
    E3 = {"tolerance": 0.4, "values": (1.0, 2.2, 4.7)}
    E6 = {"tolerance": 0.2, "values": (1.0, 1.5, 2.2, 3.3, 4.7, 6.8)}
    E12 = {"tolerance": 0.1, "values": (1.0, 1.2, 1.5, 1.8, 2.2, 2.7, 3.3, 3.9, 4.7, 5.6, 6.8, 8.2)}
    E24 = {"tolerance": 0.05, "values": (1.0, 1.1, 1.2, 1.3, 1.5, 1.6, 1.8, 2.0, 2.2, 2.4, 2.7, 3.0,
                                         3.3, 3.6, 3.9, 4.3, 4.7, 5.1, 5.6, 6.2, 6.8, 7.5, 8.2, 9.1)}
    E48 = {"tolerance": 0.02, "values": (1.00, 1.05, 1.10, 1.15, 1.21, 1.27, 1.33, 1.40, 1.47, 1.54,
    ...
    E96 = {"tolerance": 0.01, "values": (1.00, 1.02, 1.05, 1.07, 1.10, 1.13, 1.15, 1.18, 1.21, 1.24,
    ...
    E192 = {"tolerance": 0.005, "values": (1.00, 1.01, 1.02, 1.04, 1.05, 1.06, 1.07, 1.09, 1.10, 
...
def find_match(exact_val: float, sequence: VALUE_SEQUENCE) -> float:
    if sequence == VALUE_SEQUENCE.EXACT:
        return exact_val
    ...
    for val in sequence.value["values"]:
        if abs(val - exact_val) < error:
            optimal_val = val
            error = abs(val - exact_val)
    
    exact_val *= 10 ** decades
    optimal_val *= 10 ** decades
